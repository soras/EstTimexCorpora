# ===========================================================
#  Converts MThesis_2010_tml_mod_2 corpus' files to 
...
from tml_conv_utils import import_tml_file
from tml_conv_utils import _debug_concise_timex_str

from preprocessing import preprocess_for_timex_tagger

input_dir  = '../MThesis_2010_tml_mod_2/tml'
output_dir = '../MT2010tml_v1_6_json'
...
assert os.path.exists( input_dir) and os.path.isdir( input_dir ), \
       '(!) Unable to find MThesis_2010_tml_mod_2 input directory at {!r}'.format( input_dir )

# Create output dir if required
if not dry_run and not os.path.exists(output_dir):
    os.makedirs(output_dir)

# Convert all *.tml files in the input directory
original_tokens = 0
converted = 0
timex_count = 0
...
for fname in sorted(os.listdir(input_dir)):
    ...
    text_obj = import_tml_file( fpath, fname )
    if text_obj:
